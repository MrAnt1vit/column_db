...
void Converter::csvToColumnar(const std::filesystem::path &csvPath,
      ...
      [&](const std::vector<std::string> &row) {
        ...
        if (now - lastProgressTime >= std::chrono::seconds(1)) {
          ...
          double eta = (bytesPerSec > 0)
                           ? std::max(0.0, (static_cast<double>(totalBytes) -
                                            totalBytesRead) /
                                               bytesPerSec)
                           : 0.0;

          std::cout << "\r\x1b[K"
                    << "Processed " << processedRows << " rows (" << std::fixed
  ...
  std::cout << "\nConversion finished. Processed " << processedRows
            << " rows in " << totalSec << " seconds (" << std::fixed
            << std::setprecision(0) << (processedRows / totalSec)
            << " rows/s).\n";
}
...
void Converter::columnarToCsv(const std::filesystem::path &columnarPath,
  ...
  reader.forEachRow([&](const std::vector<std::string> &row) {
    ...
    if (now - lastProgress >= std::chrono::seconds(1)) {
      ...
      std::cout << "\r\x1b[K"
                << "Exported " << processedRows << " / " << totalRows
  ...
  std::cout << "\r\x1b[K"
            ...
            << totalSec << " seconds (" << std::fixed << std::setprecision(0)
            << (processedRows / totalSec) << " rows/s).\n";
}
