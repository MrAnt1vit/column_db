...
LogicalCsvReader::LogicalCsvReader(const std::filesystem::path& path, char delimiter)
...
bool compareCsvFilesLogical(const std::filesystem::path& original,
    ...
    while (true) {
        ...
        if (row1.empty() && row2.empty()) break;
        ++rowNum;
        ...
        if (showProgress) {
            ...
            if (now - lastProgress >= std::chrono::seconds(1)) {
                ...
                std::cerr << "\r\x1b[K"
                          ...
                          << std::fixed << std::setprecision(0) << rowsPerSec << " rows/s"
                          << std::flush;
                lastProgress = now;
            }
        }
        ...
        if (row1.empty() != row2.empty()) {
            if (diff) *diff = {rowNum, 0, row1.empty() ? "<EOF>" : row1[0], row2.empty() ? "<EOF>" : row2[0]};
    ...
    if (showProgress) {
        ...
        std::cerr << "\r\x1b[K"
                  << "Validation finished. Compared " << rowNum << " logical rows in "
