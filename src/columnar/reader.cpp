...
void ColumnarReader::loadMetadata() {
  ...
  char magic[4];
  in.read(magic, 4);
  if (std::string(magic, 4) != "COLM")
    throw std::runtime_error("Not a COLM file");
  uint32_t version;
  ...
  for (size_t i = 0; i < blockCount; ++i) {
    in.read(reinterpret_cast<char *>(&m_blocks[i].first),
            sizeof(m_blocks[i].first));
    in.read(reinterpret_cast<char *>(&m_blocks[i].second),
            sizeof(m_blocks[i].second));
  }
...
void ColumnarReader::forEachRow(
    std::function<void(const std::vector<std::string> &)> callback) const {
  if (!m_metadataLoaded) {
...
std::shared_ptr<ColumnData> ColumnarReader::readColumn(size_t blockIdx,
                                                       size_t columnIdx) const {
  if (!m_metadataLoaded)
    throw std::runtime_error("Metadata not loaded");
  if (blockIdx >= m_blocks.size())
    throw std::runtime_error("Invalid block index");
  if (columnIdx >= m_schema.size())
  ...
  const uint64_t entryPos = blockStart + sizeof(uint64_t) + sizeof(uint32_t) +
