...
public:
  ...
  virtual std::shared_ptr<ColumnData>
  cloneSelected(const std::vector<uint8_t> &mask, size_t outRows) const = 0;
};
...
public:
  ...
  std::shared_ptr<ColumnData> cloneSelected(const std::vector<uint8_t> &mask,
                                            size_t outRows) const override {
...
template <typename T, DataType DT> class TypedColumn : public ColumnData {
public:
  using value_type = T;
  std::vector<T> data;
...
public:
  ...
  size_t addFromString(const std::string &str) override {
    ...
    int32_t days =
        detail::days_from_epoch(tm.tm_year + 1900, tm.tm_mon + 1, tm.tm_mday);
    data.push_back(days);
    return sizeof(int32_t);
  }
...
public:
  explicit RowGroup(const Schema &schema,
                    size_t maxUncompressedBytes = 128 * 1024 * 1024);
  ...
  const std::vector<std::shared_ptr<ColumnData>> &getColumns() const {
    return m_columns;
  }
