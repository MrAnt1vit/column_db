...
std::shared_ptr<ColumnData> createColumn(DataType type) {
    switch (type) {
        case DataType::Int8:   return std::make_shared<TypedColumn<int8_t,   DataType::Int8>>();
        case DataType::Int16:  return std::make_shared<TypedColumn<int16_t,  DataType::Int16>>();
        case DataType::Int32:  return std::make_shared<TypedColumn<int32_t,  DataType::Int32>>();
        case DataType::Int64:  return std::make_shared<TypedColumn<int64_t,  DataType::Int64>>();
        case DataType::String: return std::make_shared<StringColumn>();
        case DataType::Date:   return std::make_shared<DateColumn>();
        case DataType::DateTime: return std::make_shared<DateTimeColumn>();
        case DataType::Char:   return std::make_shared<CharColumn>();
...
RowGroup::RowGroup(const Schema& schema, size_t maxUncompressedBytes)
    : m_schema(schema)
    , m_maxBytes(maxUncompressedBytes)
    , m_currentBytes(0)
    , m_rows(0)
{
