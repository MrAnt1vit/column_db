...
int doConvert(const std::string &csvPath, const std::string &schemaPath,
...
int parseConvert(int argc, char *argv[]) {
  if (argc < 5) {
    std::println(stderr, "Usage: {} convert <csv> <schema> <columnar>",
                 argv[0]);
    return 1;
  }
...
int parseRestore(int argc, char *argv[]) {
  if (argc < 4) {
    std::println(stderr, "Usage: {} restore <columnar> <csv>", argv[0]);
    return 1;
  }
...
int parseQuery(int argc, char *argv[]) {
  if (argc < 5) {
    std::println(stderr, "Usage: {} query <query_id> <columnar> <output_csv>",
                 argv[0]);
    return 1;
  }
...
int main(int argc, char *argv[]) {
  try {
    ...
    if (command == "convert")
      return parseConvert(argc, argv);
    if (command == "restore")
      return parseRestore(argc, argv);
    if (command == "query")
      return parseQuery(argc, argv);
