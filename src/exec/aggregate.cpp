...
static void sumColumnAsInt128(const Batch& b, size_t colIdx,
...
static void sumByType(const Batch& b, size_t colIdx, DataType srcType,
                      i128& accum, int64_t& count) {
    switch (srcType) {
        case DataType::Int8:  return sumColumnAsInt128<Int8Column> (b, colIdx, accum, count);
        case DataType::Int16: return sumColumnAsInt128<Int16Column>(b, colIdx, accum, count);
        case DataType::Int32: return sumColumnAsInt128<Int32Column>(b, colIdx, accum, count);
        case DataType::Int64: return sumColumnAsInt128<Int64Column>(b, colIdx, accum, count);
        default:
...
static void minMaxColumnAsInt128(const Batch& b, size_t colIdx, bool isMin,
    ...
    for (size_t i = 0; i < b.rowCount; ++i) {
        ...
        if (!hasValue) { extremum = v; hasValue = true; continue; }
        if (isMin)  { if (v < extremum) extremum = v; }
        else        { if (v > extremum) extremum = v; }
    }
}
...
static void minMaxByType(const Batch& b, size_t colIdx, DataType srcType,
    ...
    switch (srcType) {
        case DataType::Int8:     return minMaxColumnAsInt128<Int8Column>    (b, colIdx, isMin, extremum, hasValue);
        case DataType::Int16:    return minMaxColumnAsInt128<Int16Column>   (b, colIdx, isMin, extremum, hasValue);
        case DataType::Int32:    return minMaxColumnAsInt128<Int32Column>   (b, colIdx, isMin, extremum, hasValue);
        case DataType::Int64:    return minMaxColumnAsInt128<Int64Column>   (b, colIdx, isMin, extremum, hasValue);
        case DataType::Date:     return minMaxColumnAsInt128<DateColumn>    (b, colIdx, isMin, extremum, hasValue);
        case DataType::DateTime: return minMaxColumnAsInt128<DateTimeColumn>(b, colIdx, isMin, extremum, hasValue);
        default:
            throw std::runtime_error("Min/Max: unsupported source type");
    }
}

static std::shared_ptr<ColumnData> makeScalarColumn(DataType srcType, i128 value) {
    switch (srcType) {
        case DataType::Int8: {
...
GlobalAggregateOperator::GlobalAggregateOperator(std::unique_ptr<IOperator> child,
        ...
        for (size_t si = 0; si < m_specs.size(); ++si) {
            ...
            switch (spec.kind) {
                ...
                case AggSpec::Kind::Avg:
                    sumByType(*batch, spec.batchColIdx, spec.srcType,
                              st.accum, st.count);
                    break;
                ...
                case AggSpec::Kind::Min:
                    minMaxByType(*batch, spec.batchColIdx, spec.srcType,
                                 true, st.extremum, st.hasValue);
                    break;
                case AggSpec::Kind::Max:
                    minMaxByType(*batch, spec.batchColIdx, spec.srcType,
                                 false, st.extremum, st.hasValue);
                    break;
