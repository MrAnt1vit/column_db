...
public:
    ...
    void processBatch(const Batch& b, size_t srcCol,
        ...
        switch (m_srcType) {
            case DataType::Int8:  return processTyped<Int8Column> (b, srcCol, gidx);
            case DataType::Int16: return processTyped<Int16Column>(b, srcCol, gidx);
            case DataType::Int32: return processTyped<Int32Column>(b, srcCol, gidx);
            case DataType::Int64: return processTyped<Int64Column>(b, srcCol, gidx);
            default: throw std::runtime_error("Sum: unsupported source type");
        }
    }
...
public:
    ...
    void processBatch(const Batch& b, size_t /*srcCol*/,
                      const std::vector<size_t>& gidx) override {
        for (size_t i = 0; i < b.rowCount; ++i) ++m_counts[gidx[i]];
    }
...
public:
    ...
    void processBatch(const Batch& b, size_t srcCol,
        ...
        switch (m_srcType) {
            ...
            case DataType::Int32: return processTyped<Int32Column>(b, srcCol, gidx);
            case DataType::Int64: return processTyped<Int64Column>(b, srcCol, gidx);
            default: throw std::runtime_error("Avg: unsupported source type");
        }
    }
...
makeAggregateState(const AggSpec& spec) {
    switch (spec.kind) {
        case AggSpec::Kind::Sum:   return std::make_unique<SumState>(spec.srcType);
        case AggSpec::Kind::Count: return std::make_unique<CountState>();
        case AggSpec::Kind::Avg:   return std::make_unique<AvgState>(spec.srcType);
        case AggSpec::Kind::CountDistinct:
            switch (spec.srcType) {
                case DataType::Int8:     return std::make_unique<CountDistinctState<Int8Column>>();
                case DataType::Int16:    return std::make_unique<CountDistinctState<Int16Column>>();
                case DataType::Int32:    return std::make_unique<CountDistinctState<Int32Column>>();
                case DataType::Int64:    return std::make_unique<CountDistinctState<Int64Column>>();
                case DataType::Date:     return std::make_unique<CountDistinctState<DateColumn>>();
                case DataType::DateTime: return std::make_unique<CountDistinctState<DateTimeColumn>>();
                case DataType::Char:     return std::make_unique<CountDistinctState<CharColumn>>();
                case DataType::String:   return std::make_unique<CountDistinctState<StringColumn>>();
                default: throw std::runtime_error("CountDistinct: unsupported source type");
            }
...
static std::pair<std::shared_ptr<ColumnData>, size_t>
runGroupByAggregate(IOperator& child,
                    size_t keyIdx,
    ...
    while (auto batch = child.Next()) {
        ...
        for (size_t i = 0; i < batch->rowCount; ++i) {
            ...
            if (inserted) keyCol->data.push_back(k);
...
static std::pair<std::shared_ptr<ColumnData>, size_t>
dispatchGroupByAggregate(IOperator& child,
                         size_t keyIdx, DataType keyType,
                         const std::vector<AggSpec>& specs,
                         std::vector<std::unique_ptr<IGroupAggregateState>>& states) {
    switch (keyType) {
        case DataType::Int8:     return runGroupByAggregate<Int8Column>    (child, keyIdx, specs, states);
        case DataType::Int16:    return runGroupByAggregate<Int16Column>   (child, keyIdx, specs, states);
        case DataType::Int32:    return runGroupByAggregate<Int32Column>   (child, keyIdx, specs, states);
        case DataType::Int64:    return runGroupByAggregate<Int64Column>   (child, keyIdx, specs, states);
        case DataType::Date:     return runGroupByAggregate<DateColumn>    (child, keyIdx, specs, states);
        case DataType::DateTime: return runGroupByAggregate<DateTimeColumn>(child, keyIdx, specs, states);
        case DataType::Char:     return runGroupByAggregate<CharColumn>    (child, keyIdx, specs, states);
        case DataType::String:   return runGroupByAggregate<StringColumn>  (child, keyIdx, specs, states);
        default:
...
HashGroupByAggregateOperator::HashGroupByAggregateOperator(
    ...
    , m_keyIdx(keyBatchColIdx), m_keyType(keyType)
...
std::optional<Batch> HashGroupByAggregateOperator::Next() {
    ...
    for (const auto& spec : m_specs) states.push_back(makeAggregateState(spec));
