...
int q2_sumCountAvg(const std::filesystem::path &columnarPath,
  ...
  if (!advIdxOpt)
    throw std::runtime_error("Schema has no AdvEngineID");
  if (!resWIdxOpt)
    throw std::runtime_error("Schema has no ResolutionWidth");
  ...
  std::unique_ptr<IOperator> plan = std::make_unique<ScanOperator>(
      reader, std::vector<size_t>{advIdx, resWIdx});
...
int q3_avgUserId(const std::filesystem::path &columnarPath,
  ...
  if (!userIdIdxOpt)
    throw std::runtime_error("Schema has no UserID");
...
int q4_countDistinctUserId(const std::filesystem::path &columnarPath,
  ...
  std::unique_ptr<IOperator> plan =
      std::make_unique<ScanOperator>(reader, std::vector<size_t>{userIdIdx});
  plan =
      std::make_unique<CountDistinctOperator>(std::move(plan), 0, userIdType);
...
int q5_countDistinctSearchPhrase(const std::filesystem::path &columnarPath,
  ...
  if (!spIdxOpt)
    throw std::runtime_error("Schema has no SearchPhrase");
  ...
  plan = std::make_unique<CountDistinctOperator>(std::move(plan), 0, spType);
...
int q6_minMaxEventDate(const std::filesystem::path &columnarPath,
  ...
  if (!idxOpt)
    throw std::runtime_error("Schema has no EventDate");
...
int q8_regionDistinctUsers(const std::filesystem::path &columnarPath,
  ...
  if (!regionIdxOpt)
    throw std::runtime_error("Schema has no RegionID");
  if (!userIdxOpt)
    throw std::runtime_error("Schema has no UserID");
  ...
  std::unique_ptr<IOperator> plan = std::make_unique<ScanOperator>(
      reader, std::vector<size_t>{regionIdx, userIdx});
...
int q9_regionMultiAgg(const std::filesystem::path &columnarPath,
  ...
  std::unique_ptr<IOperator> plan = std::make_unique<ScanOperator>(
      reader, std::vector<size_t>{regionIdx, advIdx, resWIdx, userIdx});
